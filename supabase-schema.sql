-- ============================================================
-- PetCalendar.ai — Supabase Database Schema
-- ============================================================
...
CREATE POLICY "Users can view own pet photos"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM projects
      WHERE projects.id = pet_photos.project_id
        AND projects.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Users can view own calendar pages"
  ON calendar_pages FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM projects
      WHERE projects.id = calendar_pages.project_id
        AND projects.user_id = auth.uid()
    )
  );

CREATE POLICY "Users can insert own calendar pages"
  ON calendar_pages FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM projects
      WHERE projects.id = calendar_pages.project_id
...
CREATE POLICY "Users can update own calendar pages"
  ON calendar_pages FOR UPDATE
  USING (
    EXISTS (
      SELECT 1 FROM projects
      WHERE projects.id = calendar_pages.project_id
...
CREATE INDEX idx_pet_photos_project_id ON pet_photos(project_id);
